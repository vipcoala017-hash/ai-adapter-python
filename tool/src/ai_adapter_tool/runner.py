...
class CliStrategy:
    ...
    def invocation(self, prompt: str, cwd: Path, *, prompt_path: Path | None = None) -> AIInvocation:
        ...
        return AIInvocation(
            ...
            configured_launch_mode=self.config.launch_mode,
            effective_launch_mode=self.config.effective_launch_mode,
        )
    ...
    def run(self, prompt: str, cwd: Path, *, prompt_path: Path | None = None, stdout_path: Path | None = None, stderr_path: Path | None = None) -> AIResult:
        ...
        if "{{prompt}}" in self.config.command:
            input_text = None
        elif self.config.prompt_mode == "stdin" and prompt_path is not None and effective_launch_mode != "bash":
            input_text = None
            stdin_handle = prompt_path.open("r", encoding="utf-8")
        if self.config.prompt_mode == "arg" and effective_launch_mode == "direct" and "{{prompt}}" not in self.config.command:
            cmd.append(prompt)
            input_text = None
        ...
        try:
            ...
            raise CommandLaunchError(
                ...
                process_command=subprocess.list2cmdline(cmd),
                logical_command=command_preview(self.config),
                launch_mode=self.config.effective_launch_mode,
            ) from exc
        finally:
    ...
    def _build_command(self, prompt: str, *, prompt_path: Path | None = None) -> list[str]:
        ...
        if self.config.prompt_mode == "stdin" and not has_prompt:
            if launch_mode == "bash" and prompt_path is not None:
        ...
        raise ValueError(f"不支持的 launch_mode：{launch_mode}")
...
def replay_command(config: AIConfig, prompt_file: str = "ai-prompt.md") -> str:
    ...
    launch_mode = config.effective_launch_mode
    if "{{prompt}}" in config.command:
        if launch_mode == "powershell":
            return base.replace("<prompt>", f"(Get-Content -LiteralPath {prompt_file} -Raw)")
        return base.replace("<prompt>", f'"$(cat {shlex.quote(prompt_file)})"')
    ...
    return f'{base} "$(cat {shlex.quote(prompt_file)})"'
